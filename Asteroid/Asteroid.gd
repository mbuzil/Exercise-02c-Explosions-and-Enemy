...
func damage(d):
	...
	if health <= 0:
		collision_layer = 0
		var AsteroidContainer = get_node_or_null("/root/Game/Asteroid_Container")
		if AsteroidContainer != null:
			for s in small_asteroids:
				...
				AsteroidContainer.call_deferred("add_child", asteroid_small)
				asteroid_small.global_position = position + s.rotated(dir)
